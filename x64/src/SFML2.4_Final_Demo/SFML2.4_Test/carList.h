...
public:
	...
	void checkPositions(sf::RenderWindow& windowHandle) {
		...
		for (int i = 0; i < this->size(); i++) {
			...
			else if (tempPtr->getGlobalBounds().left < 0 - CAR_RADIUS * 2 && !carNearEdge(Lane::LaneEI)) {
			...
			else if (tempPtr->getGlobalBounds().top > windowHandle.getSize().y && !carNearEdge(Lane::LaneNI)) {
			...
			else if (tempPtr->getGlobalBounds().top < 0 - CAR_RADIUS * 2 && !carNearEdge(Lane::LaneSI)) {
...
private:
	...
	bool carNearEdge(Lane::LaneType l) {
		...
		if (diff >= CAR_RADIUS*4) { return false; } //if there is a car in the first two car spaces in lane then there is a car near edge
		else { return true; }
	}
